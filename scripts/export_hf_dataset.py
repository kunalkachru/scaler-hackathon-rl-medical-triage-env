...
def run_mock_episode(task_id: str, case_index: int) -> dict | None:
    ...
    mock_actions = {
        ...
        "icu_deterioration":         {"sofa_score": 10, "primary_organ_failure": "cardiovascular", "deterioration_trend": "worsening", "intervention": "emergency_escalation", "rationale": "High SOFA with cardiovascular failure requiring escalation"},
        "sbar_handover":             {"escalation_required": True, "priority": "critical", "assessment": "Patient is deteriorating with worsening vital signs and altered consciousness", "recommendation": "emergency_response"},
        "differential_diagnosis":    {"must_not_miss": "stemi", "top_diagnosis": "acute_coronary_syndrome", "differentials": ["unstable_angina", "pulmonary_embolism", "aortic_dissection"], "first_investigation": "ecg", "urgency": "immediate"},
    }
...
def main() -> None:
    ...
    card_path.write_text(
        ...
        "Medical Triage OpenEnv environment (v2.3.0).\n\n"
        "## Fields\n\n"
        "| Field | Description |\n"
        "|-------|-------------|\n"
        ...
        "## Grading\n\n"
        "Rewards are computed by deterministic partial-credit graders grounded in NHS NEWS2, "
        "PEWS (Duncan 2006), SOFA (Vincent 1996), SSC Hour-1 Bundle 2021, and NHS SBAR 2018.\n\n"
        "## Source\n\n"
        "Generated by `scripts/export_hf_dataset.py` from "
        "`kunalkachru23/medical-triage-env` v2.3.0.\n"
    )
